...
s    * velocity_block_with_content_list needs to be up to date in local and remote cells.
   ...
   void SpatialCell::shift_velocity_blocks(const Real minvalScale, const Real shiftScale){
      const uint8_t refLevel=0;
      for (uint popID =0; popID <  populations.size() - 1; popID++ ) {
         //shift down
               ...
               for (uint j=0; j<WID; ++j) {
                  for (uint i=0; i<WID; ++i) {
                     ...
                     if(value < minvalScale * minValue) {
                        ...
                        get_data(popID + 1)[blockLID_next * SIZE_VELBLOCK + cellIndex(i_next,j_next,k_next)] += numericalMass * shiftScale  / DV3_next;
                     }
                  }
               }
            }
